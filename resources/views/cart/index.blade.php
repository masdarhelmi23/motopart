<x-app-layout>
    {{-- Container utama halaman. Menambahkan kelas dark mode untuk background keseluruhan. --}}
    <div class="container mx-auto px-4 py-8 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 min-h-screen">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-8">Checkout Pesanan</h1> {{-- Mengubah judul menjadi "Checkout Pesanan" --}}

        @if ($cart && $cart->orderItems->count())
            {{-- Tata letak utama halaman checkout: Menggunakan grid untuk 2 kolom (kiri: konten, kanan: ringkasan) --}}
            <div class="lg:grid lg:grid-cols-3 lg:gap-8">

                {{-- Kolom Kiri: Alamat, Metode Pengiriman, Detail Barang --}}
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
                        ...
                        <div id="selected-address-display">
                            @auth
                                <p class="text-gray-700 dark:text-gray-300"><strong>{{ Auth::user()->name }}</strong></p>
                                {{-- Menampilkan alamat dari kolom 'address' di model User --}}
                                <p class="text-gray-600 dark:text-gray-400">{{ Auth::user()->address ?? 'Alamat belum diatur. Silakan atur di profil Anda.' }}</p>
                                {{-- Placeholder untuk kota/kode pos jika ada di data user/alamat --}}
                                {{-- <p class="text-gray-600 dark:text-gray-400">Salatiga, Jawa Tengah 50711</p> --}}
                        ...
                        {{-- Di sini Anda bisa menambahkan modal atau link ke halaman untuk memilih/menambah alamat --}}
                    </div>

                    {{-- 2. Bagian Metode Pengiriman --}}
                    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700" id="detail-barang-table">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 text-center text-sm text-gray-700 dark:text-gray-300">
                                            <div class="flex justify-center items-center gap-2">
                                                <button data-id="{{ $item->id }}" class="btn-qty px-2 py-1 bg-gray-200 dark:bg-gray-700 rounded text-gray-700 dark:text-gray-300" data-action="decrease">-</button>
                                                <span class="font-semibold" id="qty-{{ $item->id }}">{{ $item->quantity }}</span>
                                                <button data-id="{{ $item->id }}" class="btn-qty px-2 py-1 bg-gray-200 dark:bg-gray-700 rounded text-gray-700 dark:text-gray-300" data-action="increase">+</button>
                                            </div>
                                        </td>
                            ...
                            {{-- BARIS FOOTER TABEL UNTUK SUBTOTAL BARANG DIHILANGKAN SESUAI PERMINTAAN SEBELUMNYA --}}
                        </table>
                    </div>
                </div> {{-- Akhir Kolom Kiri --}}

                {{-- Kolom Kanan: Ringkasan Pesanan dan Tombol Pembayaran --}}
                <div class="lg:col-span-1 mt-8 lg:mt-0 space-y-6">
                    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
                        ...
                        <div class="space-y-3 text-sm">
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">Subtotal ({{ $cart->orderItems->sum('quantity') }} item)</span>
                                {{-- Berikan ID agar JS bisa memperbarui nilai ini --}}
                                <span class="text-gray-800 dark:text-gray-200" id="summary-subtotal-display">Rp {{ number_format($cart->total_amount ?? 0, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">Ongkos Kirim</span>
                                <span class="text-gray-800 dark:text-gray-200" id="shipping-cost-display">Rp 0</span> {{-- Default 0, akan diperbarui JS --}}
                            </div>
                            <div class="flex justify-between">
                                {{-- PERBAIKAN DI SINI: Label Pajak diubah menjadi 3% --}}
                                <span class="text-gray-600 dark:text-gray-400">Pajak (3%)</span>
                                <span class="text-gray-800 dark:text-gray-200" id="tax-display">Rp 0</span> {{-- Akan diperbarui JS --}}
                            </div>
                        </div>
                        <hr class="my-4 border-gray-200 dark:border-gray-700">
                        <div class="flex justify-between items-center text-lg font-bold">
                            <span class="text-gray-900 dark:text-white">Total</span>
                            <span class="text-blue-600 dark:text-blue-400" id="final-total-amount">Rp {{ number_format($cart->total_amount ?? 0, 0, ',', '.') }}</span> {{-- Default subtotal, akan diperbarui JS --}}
                        </div>
                        ...
                        <div class="mt-6">
                            <form action="{{ route('payment.process') }}" method="POST" id="checkout-form">
                                ...
                                {{-- Input tersembunyi untuk menyimpan metode pengiriman yang dipilih --}}
                                <input type="hidden" name="selected_shipping_method" id="selected-shipping-method-input">
                                {{-- Input tersembunyi untuk menyimpan total akhir termasuk ongkir dan pajak --}}
                    ...
                    {{-- Bagian Pembayaran Aman (opsional, seperti di screenshot) --}}
